...
class AdbTools(object):
    ...
    def __init__(self, device_id=''):
        ...
        device = device_id + ':' + '5555'
        if device not in self.get_devices():
            self.adb(f'connect {device}')
    ...
    def get_devices(self):
        ...
        :return:
        """
        s = self.adb('devices').readlines()
        return list((i.split()[0] for i in s if 'devices' not in i and len(i) > 5))
    ...
    def dump_apk(path):
        ...
        for i in paths:
            if 'build-tools' in i:
                build_tools = True
        if not build_tools:
            raise EnvironmentError(r"ANDROID_HOME BUILD-TOOLS COMMAND NOT FOUND.\nPlease set the environment variable.")
    ...
    def install(self, path):
        ...
        print('Installing...')
        p = self.adb('install -r %s' % (path,)).read()
        if 'Success' in p:
            print('Install Success')
        if 'Failure' in p:
            reg = re.compile(r'\\[(.+?)]')
            key = re.findall(reg, p)[0]
            try:
    ...
    def uninstall(self, package):
        ...
        """
        print('Uninstalling...')
        p = self.adb('uninstall %s' % (package,)).read()
        print(p)
    ...
    def get_device_ip_address(self):
        ...
        if not self.get_wifi_state() and not self.get_data_state():
            return
        ip = self.shell('ip addr | %s global' % self.__find).read()
        reg = re.compile(r'\d+\.\d+\.\d+\.\d+')
        return re.findall(reg, ip)[0]
    ...
    def get_device_imei(self):
        ...
        # Android 5.0以下方法
        if int(sdk) < 21:
            im = self.shell('dumpsys iphonesubinfo').read()
            reg = re.compile('[0-9]{15}')
            return re.findall(reg, im)[0]
        elif self.root():
            im = self.shell('service call iphonesubinfo 1').read()
            print(im)
            print(re.findall(re.compile("'.+?'"), im))
            imei = ''
            for i in re.findall(re.compile("'.+?'"), im):
    ...
    def get_display_state(self):
        ...
        :return: 亮屏/灭屏
        """
        s = self.shell('dumpsys power').readlines()
        for i in s:
            if 'mScreenOn=' in i:
    ...
    def get_screen_reality_size(self):
        ...
        x = 0
        y = 0
        s = self.shell(r'getevent -p | %s -e "0"' % self.__find).readlines()
        for n in s:
            if len(n.split()) > 0:
                if n.split()[0] == '0035':
    ...
    def get_device_external_sdcard(self):
        ...
        :return: (path,total,used,free,block)
        """
        return self.shell(r'df | %s \/storage' % self.__find).read().strip().split()
    ...
    def get_wifi_password_list(self):
        ...
        if not self.root():
            print('The device not root.')
            return []
        w = re.findall(re.compile(r'ssid=".+?"\s{3}psk=".+?"'), self.shell('su -c cat /data/misc/wifi/*.conf').read())
        return [re.findall(re.compile('".+?"'), i) for i in w]
